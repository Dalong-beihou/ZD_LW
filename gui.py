...
import os
import threading
from backend import monitor_network  # 假设monitor_network定义在backend模块中

# 本地模块导入
from wifi_utils import connect_to_wifi, is_connected, log

PROFILE_FILE = "wifi_profiles.json"
CONFIG_FILE = "user_settings.json"  # 用于存储用户设置的文件
...
def update_default_combo(default_combo, profiles):
    default_combo['values'] = [k for k in profiles if k != "default"]
...
def start_monitor(ssid, password):
    ...
    def run_in_thread():
        monitor_network(ssid, password, interval)  # 只传3个参数
...
def edit_profile(profiles, profile_combo, default_combo, root):
    selected = profile_combo.get()
    if selected in profiles:
...
profile_combo.bind("<<ComboboxSelected>>", lambda event: on_profile_selected(event, profile_combo, ssid_entry, pwd_entry))
update_profile_list(profile_combo, profiles)

# === 显示密码勾选框 ===
show_var = tk.IntVar()
tk.Checkbutton(root, text="显示密码", variable=show_var, command=lambda: toggle_password(pwd_entry, show_var), font=("微软雅黑", 10)).pack(pady=5)
...
def on_monitor_choice(*args):
    ...
    ssid = default_combo.get().strip()  # 获取当前默认网络
    ...
    if choice == "是":
        ...
        # 如果切换为“否”，可以在这里添加停止监控逻辑（可选）
        pass

    # 无论是否成功，都保存设置
    save_settings({"monitor_enabled": choice})

monitor_label = tk.Label(root, text="启用网络监护模式（仅作用于默认网络）", font=("微软雅黑", 10))
